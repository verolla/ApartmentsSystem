...
if($_REQUEST['sub'])
  {
    if(mysql_query("INSERT into tenants  values('','$fName','$lName','$phone','$email','$idno','$job',$age','$gender','$married','$date','$subcatg','$catg','','$pass') "))
	   {
		
	    echo "<font size='+2'>Tenant details inserted successfully to the database. They can now log in to add apartment or house details.</font>";
		}
	else
	 {
	   echo "Tenant details not inserted. Try again later.";
	   }
	}   
...
<body>
<br><br><br>
<center><h1>Add New Tenant</h1></center>
<br><br>
...
<td width="264"><select name=subcat onChange="showUser(this.value)">
 ...
 <?php
require "dbconnect1.php";// connection to database 
$q1=mysql_query("SELECT * from House");
while($n=mysql_fetch_array($q1)){
...
<tr>
  <td><span class="style3">Job:</span></td>
  <td><label>
  <input name="t2" type="text" id="t2">
  </label></td>
</tr>
